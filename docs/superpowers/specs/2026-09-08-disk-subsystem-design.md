...
- **Config format changes to match the real C — and goes through the config path that's actually live, not the one that looks like it should be.** `ConfigManager.cs`'s `GetDiskConfig()`/`DiskConfig` (`disk_image`/`disk_size`/`cache_size`/`enable_dma`) is itself invented **and has zero callers anywhere in `usim-cs`** (verified via grep) — `ConfigManager` as a whole is dead, unused parallel machinery. The config path that's actually wired into the running app is `Program.cs`'s `ApplyConfiguration(ConfigParser config)`, which reads sections directly (`config.GetString("Paths", "sys-directory", ...)` etc.) into `UsimState`'s static properties — this is what `MachineControl.LoadSystemFiles()` already reads (`UsimState.SysDirectory`) for the microcode/PROM/disk-image paths it currently hardcodes relative to. Disk config follows this same live pattern: `ApplyConfiguration` reads a real `[disk]` section (`disk0`-`disk7`, each `TYPE,filename`, e.g. `disk0 = T-300,/path/to/pack.img` — parsed the same way `disk_unit_init`'s `strtok(config_string, ",")` does, **including its one-token fallback: a value with no comma at all is treated as a bare filename with type defaulting to `"T-300"`**, not a parse error) into a new `UsimState.DiskUnits` property, and `LoadSystemFiles()` reads that instead of its current hardcoded single `disk.img` path. `ConfigManager.cs`'s dead `GetDiskConfig()`/`DiskConfig` are deleted outright, not left as a second, now-even-more-clearly-wrong invented shape sitting next to the real one. A unit with no config line, or an empty config string, stays offline — matching the real C exactly (no config-driven behavior is invented here).
...
4. `UsimConstants.cs`'s `UsimState` gains a `DiskUnits` property (an array of configured-unit type+filename pairs, empty by default). `Program.cs`'s `ApplyConfiguration` gains a loop reading `disk0`-`disk7` from a real `[disk]` section (`config.GetString("disk", $"disk{i}", "")`, split on the first comma, both parts trimmed — mirrors `disk_unit_init`'s `strtok` parsing) into `UsimState.DiskUnits`. Since `ApplyConfiguration` is currently `private static void` and needs to be callable from a test the same way `UCode`'s other internal test-only entry points already are, it becomes `internal static void` (no other visibility changes). `MachineControl.LoadSystemFiles()`'s existing hardcoded single-disk block (`string diskImage = Path.Combine(UsimState.SysDirectory, "disk.img"); ... DiskController.Mount(0, diskImage);`) is replaced with a loop over `UsimState.DiskUnits` calling `DiskController.ConfigureUnit(unit, typeName, filename)`. `ConfigManager.cs`'s dead `GetDiskConfig()` method and `DiskConfig` class are deleted (zero callers, confirmed).
...
Config test additions (`ConfigTests.cs`): a new test constructs a `ConfigParser`, loads a synthetic `[disk]` section, calls `Program.ApplyConfiguration` directly (now `internal`), and asserts `UsimState.DiskUnits` has the right type+filename pairs; a unit with no line, or an empty value, stays absent from the array (not a placeholder entry). `ConfigTests.cs` does not currently test `ApplyConfiguration` at all (verified — its existing tests only exercise `ConfigParser`/`ConfigManager` directly), so this is new coverage, not an extension of an existing test.
