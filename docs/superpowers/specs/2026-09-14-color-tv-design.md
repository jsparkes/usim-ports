...
- **VSYNC/HSYNC/sync-PROM-enable status bits (mode-read bits 5/6/7) are always 0.** Real hardware toggles them during rendering; this port never does, following the monochrome-TV precedent. This is safe because the only real consumer that reads them (`WRITE-COLOR-MAP`'s `SYNCHRONIZE=T` busy-loop) loops *while the VSYNC bit is set* and exits as soon as it reads 0 — always reporting 0 makes that loop exit immediately on its first read, never hang.
...
- Offset 0 (mode): write masks the input to `v & 0x1F` and stores it. Read returns the stored mode OR'd with three always-0 bits at positions 5 (VSYNC), 6 (HSYNC), 7 (sync-PROM-enabled) — see Decisions for why these are fixed rather than live.
