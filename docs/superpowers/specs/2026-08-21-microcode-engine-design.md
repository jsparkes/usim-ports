...
| 1 | `return (int)((SpcPtr << 24) \| (Spc[SpcPtr] & 0x1FFFFF));` |
...
| 12 | `{ int res = (int)((SpcPtr<<24) \| (Spc[SpcPtr]&0x1FFFFF)); SpcPtr = (SpcPtr - 1) & 0x1F; return res; }` |
...
- `bus_interface_bus_reset()` equivalent for `MfWrite`'s INTERRUPT-CONTROL bit-28 handling — likely a no-op or a call into `IOBus.cs`; confirm during Phase 4.
- Exact `Halted` flag location (`machine_state.halted` equivalent) for `Jmp()`'s ILLOP handling — likely `MachineControl.Halt()` or a new `UCode.Halted` field; confirm during Phase 3.
