...
- **New (Phase 8's final-review fix wave, 2026-09-03): Phase 8B.** Two items were identified during Phase 8's final review as real, independently-verified gaps, but explicitly deferred rather than fixed in this fix wave:
  - (a) A full `usim/udiss.c` port for `Disassembler.cs`: the richer `type_jump_condition()` naming (JUMP/CALL/POPJ/CALL-POPJ-?? from the p/r bits, a different condition-name vocabulary than the placeholder one currently in `DisassembleJump`), plus byte-field/A-M-I-D-memory symbolic operand decoding. The latter needs `usim-cs/SymbolTable.cs` to gain a new type dimension for AMEM/MMEM/IMEM/DMEM/NUMBER-typed symbols, and loading `sys/ubin/promh.sym`/`ucadr.sym`.
  - (b) `MainMemory`'s populated-page-count gate (`usim/main-memory.c:57-80`'s `pn < main_memory_npages` check), default 8192 pages from `usim.ini`'s `memory.size` — see the item directly above.
