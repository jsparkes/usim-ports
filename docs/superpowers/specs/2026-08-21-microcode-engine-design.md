...
- **`Disassembler.cs`**: entirely rewritten. Field extraction must use the real layout (§ above). The original version of this bullet said to source mnemonics from "the real ALU op names (§ Phase 2 tables) and jump condition names (§ Phase 3)" and to resolve DISPATCH's `disp_const` field via `defmic300.h`'s `defmics[]` table -- both are now known to be wrong. `usim/udiss.c` is a complete, already-existing reference disassembler in this repo and should be ported from directly, not re-derived from the Phase 2/3 semantic tables (which document operational behavior, not display mnemonics, and diverged from the real names in several places -- e.g. ALU case 8 is `ANDCB`, not `NOR`). `defmics[]` is a macrocode-FASL function-number table used only by `usim/unfasl*.c`; it is not applicable to microcode disassembly at all -- `usim/udiss.c:600-606`'s `dsp_const_desc()` prints `disp_const` as a plain address/`NUMBER`. Phase 8's final-review fix wave (2026-09-03) corrected the ALU (`alu_desc()`, `usim/udiss.c:308-412`), BYTE (`byt_desc()`, `usim/udiss.c:664-706`), JUMP-misc-field, and DISPATCH-map (`dsp_desc()`, `usim/udiss.c:632-645`) mnemonics from `udiss.c` directly. A full port -- `udiss.c`'s much richer `type_jump_condition()` naming (JUMP/CALL/POPJ/CALL-POPJ-?? from the p/r bits, a different condition-name vocabulary) plus byte-field/A-M-I-D-memory symbolic operand decoding (which would need `usim-cs/SymbolTable.cs` to gain a new type dimension it doesn't have) -- is deferred to a future **Phase 8B** (see Open Items below).
...
## Phase 8B — Full `udiss.c` Port + `MainMemory` Page-Count Gate

Closes the two items Phase 8's final-review fix wave (2026-09-03) identified as real, independently-verified gaps but explicitly deferred rather than fixed in that wave (see the resolved "Phase 8B" item in Open Items below). Full detail: `docs/superpowers/plans/2026-09-03-microcode-phase8b-full-udiss-port.md`.

- **`Disassembler.cs`**: ported `usim/udiss.c`'s real `type_jump_condition()` naming (JUMP/CALL/POPJ/CALL-POPJ-?? from the p/r bits, replacing the earlier placeholder vocabulary) plus shared `TypeField`/`ByteFieldOut`/`MSourceDesc`/`DestDesc` operand-decoding helpers (the full 32-entry fsource/fdest tables), wired into all four instruction classes (ALU/JUMP/DISPATCH/BYTE).
- **`SymbolTable.cs`**: gained the type dimension (AMEM/MMEM/IMEM/DMEM/NUMBER) that `udiss.c`'s symbolic operand decoding needs, and now loads the real `sys/ubin/promh.sym`/`ucadr.sym` (`--test-symbol-table`, 6/6 passing).
- **`MainMemory.cs`**: gained the populated-page-count gate (`usim/main-memory.c:57-80`'s `pn < main_memory_npages` check) — an `npages` constructor parameter (default 8192), `ReadPhysical`/`WritePhysical` gated on `pn < npages`, `pn==npages` logged at INFO ("memory probe?"), `pn>npages` logged at WARNING.
- Note: the interrupt-status-register producer divergence (`SetInterruptStatusReg`/`AssertUnibusInterrupt`/etc.) was fixed earlier, in **Phase 8's** Task 1 — that work is not part of Phase 8B, despite both being final-review fix-wave work from around the same time.

## Phase 9 — Test Suite

~~Replaces `UCodeTests.cs`'s current self-consistency-only tests (which validate the invented format against itself) with:~~ **Correction (see resolved note below): by the time this phase started, `UCodeTests.cs` was already an empty 0/0/0 placeholder, not a self-consistency suite.** The plan, as originally written:
...
**Resolved as of 2026-09-06 (Phase 9 closed):** bullets 1 (ALU/jump operation tables) and 4 (`Uvmem`'s L1/L2 same-call-visibility test) were satisfied incrementally in Phases 2/3/5, not by this phase — verified directly by both the task-level and final-plan reviews reading those test files themselves, not assumed. Phase 9 itself closed the two genuine remaining gaps: bullet 2 (`Add32`/`Sub32`/`Abs32`/`Rol32` edge cases) and bullet 3 (`TestPromDecodeSanity`'s `AAddr`/`MAddr` statistics against the real `promh.mcr`). Bullet 3's `.tbl`-cross-referencing stretch goal was deliberately not pursued: Phase 8B's `.sym`-based symbolic disassembly (tested against the real `sys/ubin/promh.sym`) already substantially achieves its intent. `UCodeTests.cs` was deleted outright rather than replaced, since its stated purpose (self-consistency-only tests validating the invented format) no longer applied by the time this phase started.
...
- ~~Exact current size/type of `DMem` in `UCode.cs` (must be `uint[2048]`) — verify and resize if needed.~~ **Resolved:** confirmed satisfied in code — `DMEM_SIZE = 2048` in `usim-cs/UCode.cs`. This was apparently always correct but never marked as verified.
- ~~Where `P1Imem`'s "PROM disabled" source flag currently lives in this port (`machine_state.promdisabled` equivalent) — likely a `MachineControl`/config flag; confirm during Phase 1.~~ **Resolved:** confirmed satisfied — `usim-cs/UCode.cs`'s `PromDisabled` field controls `P1Imem`'s fetch source, as already used throughout later phases' tests (e.g. `UCodeFetchDecodeTests.cs`'s `TestNpcWraparound`).
...
- ~~**New (found during Phase 3's final review, 2026-08-24):** `UCode`'s interrupt-pending PRODUCERS (`SetInterruptStatusReg`, `AssertUnibusInterrupt`, `AssertXbusInterrupt`, `DeassertUnibusInterrupt`, `DeassertXbusInterrupt`) diverge materially from the real C's `set_interrupt_status_reg`/`assert_unibus_interrupt`/etc. (`usim/ucode.c:114-188`)... Not assigned to any phase yet...~~ **Resolved in Phase 8's Task 1** (commit `d9a30c2`): `usim-cs/UCode.cs`'s `SetInterruptStatusReg`/`AssertUnibusInterrupt`/`DeassertUnibusInterrupt`/`AssertXbusInterrupt`/`DeassertXbusInterrupt` are now faithful ports of `usim/ucode.c:114-188` — the `0xC000` pending mask, the `0x400` enable gate, the `0x3FC` vector mask, and Xbus asserts routed through the status register — covered by `--test-microcode-interrupt` (5/5 passing).
  ~~**Escalation (Phase 4's final review, 2026-08-24):** Phase 4's `MfWrite` case 2 makes `InterruptControl` microcode-writable for the first time... this divergence is now **live, not merely latent**... this should be prioritized accordingly whenever it is finally scheduled.~~ **Resolved alongside the above:** the Phase 8 Task 1 fix covers this escalation too — both boot-PROM and `ucadr.mcr` INTERRUPT-CONTROL write sites now drive the corrected producer logic, regardless of which path sets the register.
...
- ~~**New (Phase 5's final review, 2026-08-25): `MachineControl` doesn't yet use `UCode(MainMemory)`.** It still does `Memory = new MainMemory(); UCode = new UCode();` as two independent, disconnected instances... flagging it explicitly so it isn't missed as "already done" just because the constructor exists.~~ **Resolved in Phase 8's Task 2:** `usim-cs/MachineControl.cs`'s constructor now does `Memory = new MainMemory(); UCode = new UCode(Memory);`, sharing one `MainMemory` instance.
- ~~**New (Phase 5's final review, 2026-08-25): `MainMemory.ReadPhysical`/`WritePhysical` have no populated-page-count gate.**... **Update (Phase 8's final-review fix wave, 2026-09-03): Phase 8 did NOT implement this gate.**... Re-deferred to **Phase 8B** (see below) rather than letting it silently drop.~~ **Resolved in Phase 8B's Task 1:** `usim-cs/MainMemory.cs` now has an `npages` constructor parameter (default 8192), gates `ReadPhysical`/`WritePhysical` on `pn < npages`, and distinguishes `pn==npages` ("memory probe?" INFO log) from `pn>npages` (WARNING).
- **New (Phase 5's final review, 2026-08-25): `MainMemory`'s pre-existing `Read`/`Write`/`TranslateAddress` use an invented paging scheme unrelated to `Uvmem`'s faithful L1/L2 map.** Two parallel, inconsistent virtual-memory schemes now coexist in the codebase. Reconciling or retiring the old one is Phase 8's job — see its `MachineControl.cs` bullet, which should be expanded to cover this explicitly, not just the constructor wiring. **Resolved in Phase 8's Task 2:** `MainMemory.cs` was rewritten to a flat physical-memory model (`Read`/`Write`/`ReadPhysical`/`WritePhysical` only); the invented `TranslateAddress`/paging scheme was deleted, and `MachineControl` now shares one `MainMemory` instance between `Memory` and `UCode`'s `Vm()` path.
- ~~**New (Phase 8's final-review fix wave, 2026-09-03): Phase 8B.** Two items were identified during Phase 8's final review as real, independently-verified gaps, but explicitly deferred rather than fixed in this fix wave: (a) a full `usim/udiss.c` port for `Disassembler.cs`... (b) `MainMemory`'s populated-page-count gate... see the item directly above.~~ **Resolved — (b) resolved in Phase 8B's Task 1** (see the item directly above); **(a) resolved across Phase 8B's commits:** the real `type_jump_condition()` naming (JUMP/CALL/POPJ/CALL-POPJ-?? from the p/r bits), the shared `TypeField`/`ByteFieldOut`/`MSourceDesc`/`DestDesc` operand-decoding helpers with the full 32-entry fsource/fdest tables wired into all four instruction classes, plus a real, type-dimensioned `SymbolTable.cs` (`--test-symbol-table`, 6/6 passing) loading the real `sys/ubin/promh.sym`/`ucadr.sym`. See `## Phase 8B` below for a summary.
