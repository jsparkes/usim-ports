...
| 1 (sync data) | If sync-PROM is enabled (vert-spacing bit 7 clear — see offset 3), returns the addressed sync-RAM byte; else 0 | Stores into sync-RAM at the current sync pointer, if sync-PROM is not enabled |
