...
- Mode-read bits 5 (VSYNC), 6 (HSYNC), 7 (sync-PROM-enabled) are always 0 — never toggled. This is deliberate (see spec's Decisions: it prevents a real busy-loop in `sys/window/color.lisp`'s `WRITE-COLOR-MAP` from ever hanging) and must not be "fixed" to look more hardware-accurate.
