...
void chchstart(struct chxcvr *xp);
void chchreset(struct chxcvr *xp);
 ...
 */
int chchprobe(caddr_t reg);
int chchattach(struct uba_device *ui);
int chchintr(int dev);
struct uba_device *chchdinfo[NCHCH];
unsigned short chchstd[] = { 0164140, 0165140, 0166140 };
struct uba_driver chchdriver = { chchprobe, 0, chchattach, 0, chchstd, "chch", chchdinfo };

int
chchprobe(caddr_t reg)
{
	int br, cvec;
...
int
chchattach(struct uba_device *ui)
{
 ...
 */
void
chchsetup(struct chxcvr *xp, struct ch11 *cp)
{
