...
ch_clock(void)
{
	...
	for (connp = &Chconntab[0]; connp < &Chconntab[CHNCONNS]; connp++) {
		if ((conn = *connp) == NOCONN || (conn->cn_state != CSOPEN && conn->cn_state != CSRFCSENT))
			continue;
#ifdef CHSTRCODE
		if ((pkt = conn->cn_toutput) != NOPKT && cmp_gt(Chclock, pkt->pk_time + ORATE) && !chtfull(conn)) {
		...
		if (probing) {
			...
			} else if (conn->cn_state == CSOPEN && (conn->cn_tacked != conn->cn_tlast || chtfull(conn) || inactive >= TESTRATE)) {
	...
	for (opkt = &conn->cn_thead; pkt = *opkt;)
		if (cmp_gt(Chclock, pkt->pk_time + age)) {
			...
			*opkt = pkt->pk_next;
			...
			opkt = &pkt->pk_next;
	if (firstpkt != NOPKT) {
		debug(DCONN | DABNOR, printf("Conn #%x: Rexmit (op:%d, pkn:%d)\n", CH_INDEX_SHORT(conn->cn_lidx), firstpkt->pk_op, firstpkt->pk_pkn));
		senddata(firstpkt);
	}
}
...
chroutage(void)
{
	...
	for (r = Chroutetab; r < &Chroutetab[CHNSUBNET]; r++)
		if ((r->rt_type == CHBRIDGE || r->rt_type == CHDIRECT) && r->rt_cost < CHHCOST)
			r->rt_cost++;
}
...
chbridge(void)
{
	...
	if (ndirect <= 1 || (pkt = pkalloc(n * sizeof(struct rut_data), 1)) == NOPKT)
		return;
	/*
