...
void chdrstart(struct chxcvr *xp);
void chdrreset(struct chxcvr *xp);
...
chdrinit(void)
{
	...
	for (xp = &dr11cxcvr[0]; xp < &dr11cxcvr[NDR11C]; xp++) {
		...
		dp += DR11CINC;
#endif
		 /*DEBUG*/ if (xp->xc_draddr == 0)
			panic("draddr");
		xp->xc_start = chdrstart;
		xp->xc_reset = chdrreset;
		xp->xc_reset = chxmitpkt;
		xp->xc_host = *hp++;
		LOCK;
		chdrreset(xp);
		UNLOCK;
...
chdrxtime(void)
{
	...
	for (xp = &dr11cxcvr[0]; xp < &dr11cxcvr[NDR11C]; xp++) {
		if (xp->xc_drintrup)
			continue;
		if (xp->xc_rpkt != NOPKT && Chclock - xp->xc_rtime > DR11CHUNG) {
			ch_free((char *)xp->xc_rpkt);
			xp->xc_rpkt = NOPKT;
			xp->xc_drrstate = DRIDLE;
			debug(DABNOR, printf("Rec pkt timeout\n"));
		}
		if (xp->xc_tpkt != NOPKT && Chclock - xp->xc_ttime > DR11CHUNG) {
...
chdrrint(int dev)
{
	...
	case DRCNT1:
		...
		else if ((data <<= 1) < sizeof(struct pkt_header) || data > CHMAXDATA + sizeof(struct pkt_header))
			debug(DABNOR, printf("Rec bad size: %d", data));
		else {
			LOCK;	/* in case of clock interrupt */
			if ((xp->xc_rpkt = (struct packet *)ch_alloc((int)data + sizeof(struct ncp_header), 1))
			    != NOPKT) {
