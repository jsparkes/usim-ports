...
rcvpkt(struct chxcvr *xp)
{
	...
	if (CH_ADDR_SHORT(pkt->pk_daddr) != Chmyaddr && CH_ADDR_SHORT(pkt->pk_daddr) != CH_ADDR_SHORT(xp->xc_addr) && CH_ADDR_SHORT(pkt->pk_daddr) != 0) {
		/*
		...
		if (((fwdcnt + 1) & 0x0f) == 0) {
			debug(DPKT | DABNOR, printf("Overforwarded packet\n"));
		      ignore:
			ch_free((char *)pkt);
		} else if (CH_ADDR_SHORT(pkt->pk_saddr) == Chmyaddr || CH_ADDR_SHORT(pkt->pk_saddr) == CH_ADDR_SHORT(xp->xc_addr)) {
	 ...
	 */
	else if ((index = pkt->pk_dtindex) >= CHNCONNS || ((conn = Chconntab[index]) == NOCONN) || CH_INDEX_SHORT(conn->cn_lidx) != CH_INDEX_SHORT(pkt->pk_didx)) {
		...
		case FWDOP:
			...
			break;
		default:
			debug(DPKT | DABNOR, printf("bad packet type for conn #%x in CSRFCSENT: %d\n", CH_INDEX_SHORT(conn->cn_lidx), pkt->pk_op));
...
rcvdata(struct connection *conn, struct packet *pkt)
{
	 ...
	 */
	for (npkt = pkt; npkt != NOPKT && npkt->pk_pkn == (unsigned short)(conn->cn_rlast + 1); npkt = npkt->pk_next) {
	...
	if (pkt != npkt) {
		...
		for (npkt = pkt; (npkt = npkt->pk_next) != NOPKT && cmp_gt(pkt->pk_pkn, npkt->pk_pkn);)
			pkt->pk_next = npkt;	/* save the last pkt here */
		if (npkt != NOPKT && pkt->pk_pkn == npkt->pk_pkn) {
			debug(DPKT | DABNOR, printf("Duplicate out of order packet\n"));
			pkt->pk_next = NOPKT;
			makests(conn, pkt);
			reflect(pkt);
		} else {
			...
			pkt->pk_next = npkt;
			debug(DPKT | DABNOR, printf("New out of order packet\n"));
		}
	}
}
...
receipt(struct connection *conn, unsigned short acknum, unsigned short recnum)
{
	...
	if (cmp_gt(recnum, conn->cn_trecvd)) {
		for (pktl = conn->cn_thead; pktl != NOPKT && cmp_le(pktl->pk_pkn, recnum); pktl = pkt) {
			pkt = pktl->pk_next;
			ch_free((char *)pktl);
		}
...
rcvrfc(struct packet *pkt)
{
	...
	for (conptr = &Chconntab[0]; conptr < &Chconntab[CHNCONNS];)
		if ((conn = *conptr++) != NOCONN && CH_INDEX_SHORT(conn->cn_fidx) == CH_INDEX_SHORT(pkt->pk_sidx) && CH_ADDR_SHORT(conn->cn_faddr) == CH_ADDR_SHORT(pkt->pk_saddr)) {
			if (conn->cn_state == CSOPEN) {
				debug(DPKT | DABNOR, printf("Rcvrfc: Retransmitting open chan #%x\n", CH_INDEX_SHORT((*conptr)->cn_lidx)));
				if (conn->cn_thead != NOPKT)
					chretran(conn, CHSHORTTIME);
			} else {
				debug(DPKT | DABNOR, printf("Rcvrfc: Duplicate RFC: %x\n", CH_INDEX_SHORT(conn->cn_lidx)));
			}
			ch_free((char *)pkt);
			return;
		}
	...
	if ((pktl = Chrfclist) == NOPKT)
		...
		do {
			if (CH_INDEX_SHORT(pktl->pk_sidx) == CH_INDEX_SHORT(pkt->pk_sidx) && CH_ADDR_SHORT(pktl->pk_saddr) == CH_ADDR_SHORT(pkt->pk_saddr)) {
	...
	debug(DCONN, printf("Rcvrfc: Queued Rfc on Chrfclist: '%c%c%c%c'\n", pkt->pk_cdata[0], pkt->pk_cdata[1], pkt->pk_cdata[2], pkt->pk_cdata[3]));
	RFCINPUT;
}
...
rcvrut(struct packet *pkt)
{
	...
	if (pkt->pk_ssubnet >= CHNSUBNET)
		...
		for (i = PH_LEN(pkt->pk_phead); i; i -= sizeof(struct rut_data), rd++) {
			...
			case CHFIXED:
				break;
			default:
				printf("CHAOS: Illegal chaos routing table entry %d", r->rt_type);
			}
		}
	ch_free((char *)pkt);
}
...
#ifdef pdp11
			swaplong(&sp->sb_xstat, sizeof(struct statxcvr) / sizeof(long));
#endif
			sp = (struct statdata *)((char *)sp + sizeof(struct stathead) + sizeof(struct statxcvr));
		}
	sendctl(pkt);
}
...
{
	printf("op=%s(%o) len=%d fc=%d; dhost=%o didx=%x; shost=%o sidx=%x\npkn=%d ackn=%d\n", str, pkt->pk_op, PH_LEN(pkt->pk_phead), PH_FC(pkt->pk_phead), pkt->pk_dhost, CH_INDEX_SHORT(pkt->pk_phead.ph_didx), pkt->pk_shost, CH_INDEX_SHORT(pkt->pk_phead.ph_sidx), (unsigned)pkt->pk_pkn, (unsigned)pkt->pk_ackn);
}
#endif
