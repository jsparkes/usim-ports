 ...
 *
 * 2023-2024 Bjorn Victor
 *      Fixes to make it work with cbridge, and get rid of a number of compiler warnings.
 */
#ifndef PRIVHOSTS
#define PRIVHOSTS 1
#endif
...
#include <time.h>
#include <sys/dir.h>
#if defined(linux) || defined(__OpenBSD__) || defined(__APPLE__)
...
syslog(int level, char *fmt, ...)
{
	...
	va_list args;
	va_start(args, fmt);
	vsprintf(foo, fmt, args);
	va_end(args);
	fprintf(stderr,"SYSLOG %d: %s", level, foo);
#endif
	return 0;
}
#endif
...
struct xfer {
	...
	struct xfile_handle *x_fh;	/* File handle of process */
...
struct command {
	...
	void (*c_func)(struct xfer *, struct transaction *);	/* Function to call */
 ...
 */
struct xfile_handle {
	struct xfile_handle *f_next;	/* Next on global list */
	char *f_name;		/* Identifier from user end */
	int f_type;		/* See below */
#ifdef SELECT
	struct connection *f_connection;
#else
	int f_fd;		/* UNIX file descriptor */
#endif
	struct xfer *f_xfer;	/* Active xfer on this fh */
	struct xfile_handle *f_other;	/* Other fh of in/out pair */
}          *xfile_handles;

#define	FNULL	((struct xfile_handle *)0)
...
#define O_XDIRECTORY	BIT(12)	/* Used internally - not a keyword option */
...
#define FALSUM		"NIL"	/* False value in binary options */
#define TRUTH		"T"	/* True value in binary options */
 ...
 * Command definitions
 */
void dataconn(struct xfer *unused, struct transaction *t);
void undataconn(struct xfer *unused, struct transaction *t);
void fileopen(struct xfer *unused, struct transaction *t);
void fileclose(struct xfer *x, struct transaction *t);
void filepos(struct xfer *x, struct transaction *t);
void delete(struct xfer *unused, struct transaction *t);
void xrename(struct xfer *unused, struct transaction *t);
void filecontinue(struct xfer *x, struct transaction *t);
void login(struct xfer *unused, struct transaction *t);
void directory(struct xfer *unused, struct transaction *t);
void complete(struct xfer *unused, struct transaction *t);
void chngprop(struct xfer *unused, struct transaction *t);
void crdir(struct xfer *unused, struct transaction *t);
void expunge(struct xfer *unused, struct transaction *t);
void getprops(struct xfer *unused, struct transaction *t);
...
void renfh(struct xfer *x, struct transaction *t);
 ...
 */
char *getname(struct stat *s, char *cp), *getdev(struct stat *s, char *cp), *getprot(struct stat *s, char *cp), *getblock(struct stat *s, char *cp), *getspace(struct stat *s, char *cp), *getbyte(struct stat *s, char *cp), *xgetbsize(struct stat *s, char *cp), *getsize(struct stat *s, char *cp), *getcdate(struct stat *s, char *cp), *getmdate(struct stat *s, char *cp), *getrdate(struct stat *s, char *cp), *getdir(struct stat *s, char *cp);
char *getsprops(struct stat *s, char *cp);
int putname(struct stat *s, char *file, char *newname, struct xfer *x), putprot(struct stat *s, char *file, char *newprot, struct xfer *x), putmdate(struct stat *s, char *file, char *newtime, struct xfer *x), putrdate(struct stat *s, char *file, char *newtime, struct xfer *x);
...
struct property {
	...
	char *(*p_get)(struct stat *s, char *cp);	/* Function to get the property */
	int (*p_put)(struct stat *s, char *file, char *new, struct xfer *x);		/* Function to set the property */
} properties[] = {
/*	name				flags						 */
 ...
 * Globals
 */
static struct chstatus chst;		/* Status buffer for connections */
...
static char *find_case_insensitive(char *directory, char *name);
static char *getatdepth(char *path, int level);
char *case_insensitive_path(char *path);

/*long lseek(), fseek(), tell();*/
#include <pwd.h>
char *tempfile(char *dname);
#include <time.h>
struct transaction *makecmd(struct command *c, struct xfile_handle *f);
struct transaction *
#ifdef SELECT
getwork(struct connection *curconn); // avoid global vars when threading, please
#else
getwork(void);
#endif
char **glob(char *v);
...
void xcommand(struct transaction *);
int parseargs(unsigned char *args, struct command *c, struct transaction *t);
int string(unsigned char **args, char *term, unsigned char **dest);
int getxoption(struct cmdargs *a, int xvalue, char **args);
...
#if defined(MAP_ROOT_DIRECTORY)
char *treeroot = NULL;
char *treeroot_prefix = NULL;

void
settreeroot(const char *root, const char *prefix)
{
	treeroot = strdup(root);
	if (prefix != NULL)
		treeroot_prefix = strdup(prefix);
	if (treeroot[strlen(treeroot)] == '/')
		treeroot[strlen(treeroot)] = '\0';
}

/* Hide the treeroot from the name. Caller should free() the result. */
char *
hidetreeroot(char *realname)
{
	char *realreal = malloc(strlen(realname)+(treeroot_prefix != NOSTR ? strlen(treeroot_prefix) : 0)+2);
	if (realreal == NOSTR)
		fatal(NOMEM);
	char *rn = realname;
	if ((treeroot != NULL) && (strncmp(rn,treeroot,strlen(treeroot)) == 0)) {
		rn += strlen(treeroot);
		if (treeroot_prefix != NULL)
			sprintf(realreal, "%s%s", treeroot_prefix, rn);
		else if (strlen(rn) == 0)
			strcpy(realreal,"/");
		else
			strcpy(realreal, rn);
...
_processdata(void *conn)
{
	...
	mylogin.cl_haddr = uch11_myaddr;
	while ((t = getwork((struct connection *)conn))) {
		if (t->t_command->c_flags & C_XFER)
			xcommand(t);
		else
			(*t->t_command->c_func) (XNULL, t);
	}
...
main(int argc, char **argv)
{
	...
	chstatus(0, &chst);
#if defined(__OpenBSD__)
	mylogin.cl_atime = timeval.tv_sec;
	mylogin.cl_ltime = timeval.tv_sec;
#else
	...
	if ((ufd = open(FILEUTMP, 1)) >= 0) {
		lseek(ufd, (long)(mylogin.cl_cnum * sizeof(struct chlogin)), 0);
		write(ufd, (char *)&mylogin, sizeof(mylogin));
		close(ufd);
	}
	signal(SIGTERM, sigfin);
	signal(SIGHUP, SIG_IGN);
	while ((t = getwork()) != NULL)
		if (t->t_command->c_flags & C_XFER)
			xcommand(t);
		else
			(*t->t_command->c_func) (XNULL, t);
...
#endif
{
	...
	for (;;) {
		...
		packet = chaos_connection_dequeue(curconn);
		if (packet == NOPKT) {
			tfree(t);
			return (TNULL);
		}
		t->t_connection = curconn;
		length = PH_LEN(packet->pk_phead) & 0x3ff;
		p.cp_op = packet->pk_op;
		memcpy(p.cp_data, packet->pk_cdata, strlen(packet->pk_cdata));
#else
...
#endif
#if 1
		syslog(LOG_INFO, "FILE: pkt(%d):%.*s\n", p.cp_op & 0377, length - 1, p.cp_data);
#endif
		switch (p.cp_op) {
		case EOFOP:
			tfree(t);
...
#ifdef SELECT
			t->t_connection->cn_state = CSCLOSED;
#endif
			tfree(t);
			return TNULL;
		case RFCOP:
		...
		*cp = '\0';
		...
		if (*++cp != CHSP) {
			struct xfile_handle *f;

			for (fhname = (char *)cp; *++cp != CHSP;)
				if (*cp == '\0') {
					errstring = "No command after file handle";
					goto cmderr;
				}
			*cp = '\0';
			for (f = xfile_handles; f; f = f->f_next)
				if (strncmp(fhname, f->f_name, FHLEN) == 0)
					break;
			if (f == FNULL) {
				sprintf(errstring = errbuf, "Unknown file handle: %s", fhname);
				goto cmderr;
			}
			t->t_fh = f;
		}
		for (cname = (char *)++cp; *cp != '\0' && *cp != CHSP && *cp != CHNL;)
			cp++;
		save = *cp;
		*cp = '\0';
		if (*cname == '\0') {
		...
		for (c = commands; c->c_name; c++)
			if (strcmp(cname, c->c_name) == 0)
				break;
		if (c == CNULL) {
			sprintf(errstring = errbuf, "Unknown command: %s", cname);
			errcode = UKC;
			goto cmderr;
		}
		...
		*cp = save;
		if (t->t_fh == FNULL) {
			if (c->c_flags & (C_FHMUST | C_FHINPUT | C_FHOUTPUT)) {
				sprintf(errstring = errbuf, "Missing file handle on command: %s", cname);
				goto cmderr;
			}
		} else if (c->c_flags & C_FHCANT) {
			sprintf(errstring = errbuf, "File handle present on command: %s", cname);
			goto cmderr;
		} else if ((c->c_flags & C_FHINPUT && t->t_fh->f_type != F_INPUT) || (c->c_flags & C_FHOUTPUT && t->t_fh->f_type != F_OUTPUT)) {
			sprintf(errstring = errbuf, "Wrong direction file handle (%s) for command: %s", fhname, cname);
			goto cmderr;
		}
		if (*cp == ' ')
			cp++;
...
parseargs(unsigned char *args, struct command *c, struct transaction *t)
{
	...
	for (syntax = c->c_syntax; *syntax != END;) {
		...
		case SP:
			if (*args++ != CHSP) {
				sprintf(errstring = errbuf, "Space expected where 0%o (octal) occurred", args[-1]);
				goto synerr;
			}
			continue;
		case NL:
			if (*args++ != CHNL) {
				sprintf(errstring = errbuf, "Newline expected where 0%o (octal) occurred", args[-1]);
				goto synerr;
			}
			continue;
		case OPEND:
		...
		case NUMBER:
			if (!isdigit(*args)) {
				sprintf(errstring = errbuf, "Digit expected where 0%o (octal) occurred", args[0]);
				goto synerr;
			}
			n = 0;
			while (isdigit(*args)) {
			...
			continue;
		case STRING:
			if ((errcode = string(&args, syntax, (unsigned char **) &a->a_strings[nstrings++])) != 0)
				goto synerr;
			continue;
		case PNAME:
			if ((p = a->a_plist) != PNULL && p->p_value == NOSTR) {
				errstring = "Property name expected when already given";
				goto synerr;
			}
			if ((p = salloc(plist)) == PNULL)
				fatal(NOMEM);
			if ((errcode = string(&args, syntax, (unsigned char **) &p->p_name)) != 0) {
				sfree(p);
				goto synerr;
			}
			p->p_next = a->a_plist;
		...
		case PVALUE:
			if ((p = a->a_plist) == PNULL || p->p_value != NOSTR) {
				errstring = "Property value expected when no name given";
				goto synerr;
			}
			if ((errcode = string(&args, syntax, (unsigned char **) &p->p_value)) != 0)
				goto synerr;
			continue;
		case OPTIONS:
			while (*args != '\0' && *args != CHNL) {
				...
				while (*args == ' ')
					args++;
				if ((errcode = string(&args, "", (unsigned char **) &oname)) != 0)
					goto synerr;
				if (*oname == '\0')
					continue;
				...
				if (o->o_name == NOSTR) {
					...
					sprintf(errstring = errbuf, "Unknown open option: %s", oname);
					return UUO;
				}
				switch (o->o_type) {
				case O_BIT:
					...
					break;
				case O_NUMBER:
					if (*args++ != CHSP || !isdigit(*args)) {
						sprintf(errstring = errbuf, "Digit expected where 0%o (octal) occurred", args[-1]);
						goto synerr;
					}
					n = 0;
					while (isdigit(*args)) {
					...
					break;
				case O_EXISTS:
					if ((errcode = getxoption(a, o->o_value, (char **) &args)) != 0)
						goto synerr;
					break;
				}
			}
			for (o = options; o->o_name; o++)
				if (o->o_type == O_BIT && (o->o_value & a->a_options) != 0 && (o->o_cant & a->a_options)) {
					errcode = ICO;
					goto synerr;
				}
			continue;
...
int
getxoption(struct cmdargs *a, int xvalue, char **args)
{
	struct xoption *x;
	int errcode;
	char *xname;
	...
	if ((errcode = string((unsigned char **) args, "", (unsigned char **) &xname)) != 0)
		return errcode;
	if (*xname == '\0') {
 ...
 */
int
string(unsigned char **args, char *term, unsigned char **dest)
{
	unsigned char *cp;
	unsigned char *s;
...
respond(struct transaction *t, char *results)
{
	...
	p.cp_op = DATOP;
	sprintf(p.cp_data, "%s %s %s%s%s", t->t_tid, t->t_fh != FNULL ? t->t_fh->f_name : "", t->t_command->c_name, results != NOSTR ? " " : "", results != NOSTR ? results : "");
#ifdef SELECT
	struct packet *chpkt = chaos_allocate_packet(t->t_connection, p.cp_op, (int)strlen(p.cp_data));

	memcpy(chpkt->pk_cdata, p.cp_data, strlen(p.cp_data));
#if CBDEBUG
	fprintf(stderr,"%s: %s [%p]\n", __func__, p.cp_data, (void *)pthread_self());
#endif
	chaos_connection_queue(t->t_connection, chpkt);
#else
	int len = 1 + strlen(p.cp_data);
	{
		int ret;
...
error(struct transaction *t, char *fh, int code)
{
	...
	p.cp_op = DATOP;
	sprintf(p.cp_data, "%s %s ERROR %s %c %s", t->t_tid, fh, e->e_code, errtype ? errtype : E_COMMAND, errstring ? errstring : e->e_string);
#if 1
	syslog(LOG_INFO, "FILE: %s\n", p.cp_data);
#endif
	errstring = NOSTR;
 ...
 */
int
syncmark(struct xfile_handle *f)
{
	char op;
	...
	op = SYNOP;
#ifdef SELECT
	struct packet *chpkt = chaos_allocate_packet(f->f_connection, op, 0);

	chaos_connection_queue(f->f_connection, chpkt);
#else
 ...
 */
void
dataconn(struct xfer *unused, struct transaction *t)
{
	struct xfile_handle *ifh, *ofh;
	 ...
	 */
	for (ifh = xfile_handles; ifh; ifh = ifh->f_next)
		if (strcmp(ifhname, ifh->f_name) == 0 || strcmp(ofhname, ifh->f_name) == 0) {
...
#ifndef USE_CBRIDGE
#ifndef SELECT
	    || chwaitfornotstate(fd, CSRFCSENT) < 0 ||	/* Hangs here */
	    chstatus(fd, &chst) < 0 || chst.st_state != CSOPEN || chsetmode(fd, CHRECORD)
#endif
#endif
		) {
#ifndef SELECT
	...
	if ((ifh = salloc(xfile_handle)) == FNULL || (ofh = salloc(xfile_handle)) == FNULL) {
		fatal(NOMEM);
		return;
	}
#ifdef SELECT
 ...
 */
void
undataconn(struct xfer *x, struct transaction *t)
{
	struct xfile_handle *f, *of;

	f = t->t_fh;
	if (f->f_xfer != XNULL || f->f_other->f_xfer != XNULL) {
		errstring = "The data connection to be removed is still in use";
...
#endif
		if (xfile_handles == f)
			xfile_handles = f->f_next;
		else {
			for (of = xfile_handles; of->f_next != f; of = of->f_next)
				if (of->f_next == FNULL)
					fatal(BADFHLIST);
			of->f_next = f->f_next;
		}
		if (xfile_handles == f->f_other)
			xfile_handles = f->f_other->f_next;
		else {
			for (of = xfile_handles; of->f_next != f->f_other; of = of->f_next)
				if (of->f_next == FNULL)
					fatal(BADFHLIST);
...
#ifdef SELECT
		/*
		 * Need to respond before we free the file handle. 
		 */
		respond(t, NOSTR);
		rlsconn(f->f_connection);
		f->f_connection = NULL;
#endif
		free(f->f_name);
		of = f->f_other;
...
char *privileged_hosts[] = { "mit-tweety-pie", "mit-daffy-duck", "cadr" };
 ...
 */
void
login(struct xfer *unused, struct transaction *t)
{
	struct passwd *p;
	struct cmdargs *a;
	int ufd;
 ...
 */
	...
	if ((name = a->a_strings[0]) == NOSTR) {
		...
		error(t, "", BUG);
#endif
	} else if (strcasecmp("lispm", name) == 0) {
		int uid;

		uid = getuid();
		syslog(LOG_INFO, "FILE: login() uid = %d\n", uid);
		p = getpwuid(uid);
		if (p == (struct passwd *)0) {
			syslog(LOG_INFO, "FILE: login() failed to get uid of current user '%d'\n", uid);
...
/*      allow logins without passwords from privileged hosts */
	...
	loginpwok:
...
#if defined(MAP_ROOT_DIRECTORY)
		if ((strcasecmp(name,"lispm") == 0) &&
		    (treeroot != NULL) && (treeroot_prefix == NULL))
			/* Use / for lispm, to keep it simple and avoid No such directory */
			home  = savestr("/");
		else
		...
		cwd = savestr(home);
		umask(0);
#if defined(BSD42) || defined(linux) || defined(__OpenBSD__) || defined(__APPLE__)
...
#if defined(MAP_ROOT_DIRECTORY)
		char *rn = hidetreeroot(home);
		sprintf(response, "%s %s/%c%s%c", name, rn, CHNL, fullname(p), CHNL);
		free(rn);
#else
		sprintf(response, "%s %s/%c%s%c", name, home, CHNL, fullname(p), CHNL);
#endif
		respond(t, response);
		syslog(LOG_NOTICE, "FILE: logged in as %s from host %s\n", mylogin.cl_user, chaos_name(mylogin.cl_haddr));
		strncpy(mylogin.cl_user, name, sizeof(mylogin.cl_user));
		if ((ufd = open(FILEUTMP, 1)) >= 0) {
			lseek(ufd, (long)(mylogin.cl_cnum * sizeof(struct chlogin)), 0);
			write(ufd, (char *)&mylogin, sizeof(mylogin));
			close(ufd);
...
#if 0
		{
			...
			sprintf(str, "/usr/local/f @%s", chaos_name(mylogin.cl_haddr));
			strcat(str, " |/usr/local/logger -t FILE -p %d", LOG_NOTICE);
			system(str);
		}
#endif
	}
 ...
 */
void
fileopen(struct xfer *unused, struct transaction *t)
{
	struct xfile_handle *f = t->t_fh;
	struct xfer *x;
	int errcode, fd;
	...
	char *realname = NOSTR, *dirname = NOSTR, *qfasl = FALSUM;
	char *tempname = NOSTR;
	...
	if ((errcode = parsepath(pathname, &dirname, &realname, 0)) != 0)
		goto openerr;
	printf("FILE: realname = %s\n", realname);
	if (options & ~(O_RAW | O_READ | O_WRITE | O_PROBE | O_DEFAULT | O_PRESERVE | O_BINARY | O_CHARACTER | O_PROBEDIR)
	    ) {
	...
	case O_PROBE:
		if (stat(realname, &sbuf) < 0) {
		  ...
		  /* Check if it is QFASLP, also for probe */
		  fd = open(realname, O_RDONLY);
		  if (fd < 0)
		    fd = 0;	/* never mind */
		  else  {
		    unsigned short ss[2];
		    if (read(fd, (char *)ss, sizeof(ss)) == sizeof(ss) && ((ss[0] == QBIN1 && ss[1] == QBIN2) || (ss[0] == LBIN1 && ss[1] < LBIN2LIMIT))) {
		      options |= O_BINARY;
		    } else {
		      options |= O_CHARACTER;
		    }
		...
		if (stat(realname, &sbuf) == 0) {
			...
			if (errno != ENOENT || access(dirname, 1) != 0)
				...
				case O_XERROR:
					...
					break;
				case O_XCREATE:
					if (ifexists == O_XAPPEND || ifexists == O_XOVERWRITE || ifexists == O_XTRUNCATE)
						fd = creat(realname, 0644);
					break;
				}
		}
	...
	if (tm->tm_year > 99)
		tm->tm_year %= 100;
#endif
	nbytes = options & O_CHARACTER || bytesize <= 8 ? sbuf.st_size : (sbuf.st_size + 1) / 2;
#if defined(MAP_ROOT_DIRECTORY)
	/* Hide the treeroot from the name */
	char *rn = hidetreeroot(realname);
	if (protocol > 0)
		sprintf(response, "%02d/%02d/%02d %02d:%02d:%02d %ld %s%s%s%c%s%c", tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, nbytes, qfasl, options & O_DEFAULT ? " " : "", options & O_DEFAULT ? (options & O_CHARACTER ? TRUTH : FALSUM) : "", CHNL, rn, CHNL);
	else
		sprintf(response, "%d %02d/%02d/%02d %02d:%02d:%02d %ld %s%c%s%c", -1, tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, nbytes, qfasl, CHNL, rn, CHNL);
	free(rn);
#else
	if (protocol > 0)
		sprintf(response, "%02d/%02d/%02d %02d:%02d:%02d %ld %s%s%s%c%s%c", tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, nbytes, qfasl, options & O_DEFAULT ? " " : "", options & O_DEFAULT ? (options & O_CHARACTER ? TRUTH : FALSUM) : "", CHNL, realname, CHNL);
	else
		sprintf(response, "%d %02d/%02d/%02d %02d:%02d:%02d %ld %s%c%s%c", -1, tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, nbytes, qfasl, CHNL, realname, CHNL);
#endif
...
tempfile(char *dname)
{
	char *cp = malloc((unsigned)(strlen(dname) + 2 + sizeof("#FILEpppppdd")));
	int i;
	static int lastnum;
	...
	for (i = 0; i < 100; i++) {
		...
		if (uniq > 99)
			uniq -= 100;
		sprintf(cp, "%s/#FILE%05d%02d", dname, mypid, uniq);
		if (access(cp, 0) != 0) {
			if (errno == ENOENT) {
				/*
...
void
getprops(struct xfer *unused, struct transaction *t)
{
	...
	if (t->t_fh->f_xfer != XNULL) {
		...
		if (f == FNULL) {
			errcode = BUG;
			sprintf(errstring = errbuf, "Unknown file handle: %s", a->a_strings[0]);
		} else if ((x = f->f_xfer) == XNULL) {
			errcode = BUG;
			sprintf(errstring = errbuf, "File handle: %s has no associated file", a->a_strings[0]);
		} else {
...
propopen(struct xfer *x, struct transaction *t)
{
	...
	*cp++ = '\n';
#if defined(MAP_ROOT_DIRECTORY)
	char *rn = hidetreeroot(x->x_realname);
	strcpy(cp, rn);
	free(rn);
#else
 ...
 */
void
directory(struct xfer *unused, struct transaction *t)
{
	struct cmdargs *a = t->t_args;
	struct xfer *x;
	int errcode = 0;
	...
	if (a->a_options & ~(O_DELETED | O_FAST | O_SORTED)) {
		...
		if (!(a->a_options & (O_BINARY | O_CHARACTER)))
			a->a_options |= O_CHARACTER;
		x = makexfer(t, a->a_options | O_XDIRECTORY);
		x->x_realname = realname;
		x->x_dirname = dirname;
		x->x_state = X_IDLE;
		if ((errcode = startxfer(x)) == 0) {
 ...
 */
void
xqueue(struct transaction *t, struct xfer *x)
{
...
{
	x->x_flags |= X_CLOSE;
	switch (x->x_options & (O_READ | O_WRITE | O_XDIRECTORY | O_PROPERTIES)) {
	case O_READ:
	case O_XDIRECTORY:
	case O_PROPERTIES:
		/*
...
xclose(struct xfer *ax)
{
	...
	write(ctlpipe[1], (char *)&ax, sizeof(x));
#endif
	if (x->x_options & (O_XDIRECTORY | O_PROPERTIES)) {
		respond(t, NOSTR);
		return;
	}
	/*
	 ...
	 */
	if (x->x_options & O_WRITE && x->x_tempname && !(x->x_flags & X_DELETE)) {
		/*
		...
		if (link(x->x_tempname, x->x_realname) == 0)
			...
			case ENOENT:
				...
				break;
			case EEXIST:
#if 0 // BV: backups are out-of-spec, and not wanted or needed
				backfile(x->x_realname);
#else
				// Need to remove the old before moving in the new.
				unlink(x->x_realname);
#endif
				if (link(x->x_tempname, x->x_realname) == 0)
					unlink(x->x_tempname);
	...
	if (errcode == 0) {
		if (fstat(x->x_fd, &sbuf) < 0) {
			syslog(LOG_ERR, "fd:%d, pid:%d, mypid:%d, errno:%d\n", x->x_fd, getpid(), mypid, errno);
			fatal("Fstat in xclose 1");
		}
		if (x->x_options & O_PRESERVE || x->x_flags & (X_ATIME | X_MTIME)) {
#if defined(linux) || defined(__OpenBSD__) || defined(__APPLE__)
			struct timeval timep[2];

			timep[0].tv_sec = (x->x_options & O_PRESERVE || x->x_flags & X_ATIME) ? x->x_atime : sbuf.st_atime;
			timep[1].tv_sec = (x->x_options & O_PRESERVE || x->x_flags & X_MTIME) ? x->x_mtime : sbuf.st_mtime;
#else
			time_t timep[2];

			timep[0] = (x->x_options & O_PRESERVE || x->x_flags & X_ATIME) ? x->x_atime : sbuf.st_atime;
			timep[1] = (x->x_options & O_PRESERVE || x->x_flags & X_MTIME) ? x->x_mtime : sbuf.st_mtime;
#endif
			/*
			...
			if (utimes(x->x_realname, timep)) {
				syslog(LOG_INFO, "error from utimes: errno = %d %s\n", errno, strerror(errno));
			}
#else
		...
		if (tm->tm_year > 99)
			tm->tm_year %= 100;
#endif
// @@@@ Note for format string warnings below: sbuf.st_size is an off_t, which on macOS is a 'long long' but on Raspian is 'long'.
// Using '%lld' gives segmentation fault on Rasbian.
#if defined(MAP_ROOT_DIRECTORY)
		char *rn = hidetreeroot(x->x_realname);
		if (protocol > 0)
			sprintf(response, "%02d/%02d/%02d %02d:%02d:%02d %ld%c%s%c", tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, sbuf.st_size, CHNL, rn, CHNL);
		else
			sprintf(response, "%d %02d/%02d/%02d %02d:%02d:%02d %ld%c%s%c", -1, tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, sbuf.st_size, CHNL, rn, CHNL);
		free(rn);
#else
		if (protocol > 0)
			sprintf(response, "%02d/%02d/%02d %02d:%02d:%02d %ld%c%s%c", tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, sbuf.st_size, CHNL, x->x_realname, CHNL);
		else
			sprintf(response, "%d %02d/%02d/%02d %02d:%02d:%02d %ld%c%s%c", -1, tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec, sbuf.st_size, CHNL, x->x_realname, CHNL);
#endif
		respond(t, response);
	} else
...
filepos(struct xfer *x, struct transaction *t)
{
	if ((x->x_options & O_READ) == 0) {
	...
	} else if (!(x->x_state == X_PROCESS || x->x_state == X_REOF || x->x_state == X_SEOF)) {
		...
		if (new < 0 || new > size) {
			...
			syslog(LOG_INFO, "pid: %ld, x: %X, fd: %ld, size: %ld, old: %ld, new: %ld, pos: %ld\n", getpid(), x, x->x_fd, size, old, new, tell(x->x_fd));
		}
	}
}
 ...
 */
void
delete(struct xfer *unused, struct transaction *t)
{
	struct xfile_handle *f = t->t_fh;
	struct xfer *x;
	...
	if (f != FNULL)
		if (t->t_args != ANULL && t->t_args->a_strings[0] != NOSTR) {
			errstring = "Both a file handle and filename in DELETE";
			error(t, f->f_name, BUG);
		} else if ((x = f->f_xfer) == XNULL) {
			errstring = "No transfer when DELETE on file handle";
			error(t, f->f_name, BUG);
		} else if (f->f_xfer->x_options & (O_PROPERTIES | O_XDIRECTORY)) {
			errstring = "Trying to DELETE a directory list transfer";
			error(t, f->f_name, BUG);
		} else {
			fhname = f->f_name;
			...
			f->f_xfer->x_flags |= X_DELETE;
#endif
	} else if (t->t_args == ANULL || (file = t->t_args->a_strings[0]) == NOSTR) {
		...
		if (access(dir, 2) != 0) {
			...
			if ((pid = fork()) == 0) {
				...
				dup(0);
				dup(0);
				execl("/bin/rmdir", "rmdir", real, NULL);
				execl("/usr/bin/rmdir", "rmdir", real, NULL);
				exit(1);
			} else if (pid == -1) {
				...
				if (st != 0) {
					 ...
					 * the reason but...
					 */
					errstring = "Directory to delete is not empty.";
					error(t, "", DNE);
				} else
					respond(t, NOSTR);
			}
 ...
 */
void
xrename(struct xfer *unused, struct transaction *t)
{
	struct xfile_handle *f;
	struct xfer *x;
	int errcode;
	char *file1, *file2, *dir1 = NOSTR, *dir2 = NOSTR, *real1 = NOSTR, *real2 = NOSTR;
	...
	if ((errcode = parsepath(file1, &dir1, &real1, 0)) != 0)
		...
		if (file2 != NOSTR) {
			errstring = "Both file handle and file name specified on RENAME";
			error(t, f->f_name, BUG);
		} else if ((x = f->f_xfer) == XNULL) {
			errstring = "No transfer on file handle for RENAME";
			error(t, f->f_name, BUG);
		} else if (x->x_options & (O_XDIRECTORY | O_PROPERTIES)) {
			...
			if (x->x_options & O_WRITE) {
				...
				errcode = mv(x->x_realname, x->x_dirname, real1, dir1);
			if (errcode)
...
void
complete(struct xfer *unused, struct transaction *t)
{
	char *cp, *tp;
	...
	char *dfile, *ifile, *ddir, *idir, *dreal, *ireal, *dname, *iname, *dtype, *itype, *adir, *aname, *atype;
	union {
		struct direct de;
	...
	if (dfile[0] != '/') {
		errstring = "Default for completion is not an absolute pathname";
		error(t, "", IPS);
		...
		if (ifile[0] != '/') {
			...
			strcat(adir, "/");
			strcat(adir, ifile);
			if ((errcode = parsepath(adir, &idir, &ireal, 1)) != 0) {
				error(t, "", errcode);
				goto freeall;
			}
			free(adir);
			adir = idir;
			idir = NOSTR;
		} else if ((errcode = parsepath(ifile, &adir, &ireal, 1)) != 0) {
			error(t, "", errcode);
			goto freeall;
		}
		...
		syslog(LOG_INFO, "ifile:'%s'\nireal:'%s'\nidir:'%s'\n", ifile ? ifile : "!", ireal ? ireal : "!", idir ? idir : "!");
		syslog(LOG_INFO, "dfile:'%s'\ndreal:'%s'\nddir:'%s'\n", dfile ? dfile : "!", dreal ? dreal : "!", ddir ? ddir : "!");
		...
		while (read(dfd, (char *)&d.de, sizeof(d.de)) == sizeof(d.de)) {
			...
			if (d.de.d_ino == 0 || (d.de.d_name[0] == '.' && (d.de.d_name[1] == '\0' || (d.de.d_name[1] == '.' && d.de.d_name[2] == '\0'))))
				continue;
			ename = d.de.d_name;
#else
		while ((dirp = readdir(dfd)) != NULL) {
			...
			if (dirp->d_ino == 0 || (dirp->d_name[0] == '.' && (dirp->d_name[1] == '\0' || (dirp->d_name[1] == '.' && dirp->d_name[2] == '\0'))))
				continue;
			ename = dirp->d_name;
#endif
			if ((etype = rindex(ename, '.')) != NOSTR)
				*etype++ = '\0';
			if ((namematch = prefix(iname, ename)) == SNONE || (typematch = prefix(itype, etype)) == SNONE)
				continue;
			syslog(LOG_INFO, "ename:'%s'\netype:'%s'\n", ename ? ename : "!", etype ? etype : "!");
...
#if defined(MAP_ROOT_DIRECTORY)
		char *ad = adir ? hidetreeroot(adir) : NOSTR;
		sprintf(errbuf, "%s%s%s%s%s", ad ? ad : "", ad && ad[1] != '\0' ? "/" : "", iname ? iname : "", itype ? "." : "", itype ? itype : "");
		if (ad) free(ad);
#else
		sprintf(errbuf, "%s%s%s%s%s", adir ? adir : "", adir && adir[1] != '\0' ? "/" : "", iname ? iname : "", itype ? "." : "", itype ? itype : "");
#endif
		if ((nstate == SEXACT || nstate == SPREFIX) && (tstate == SEXACT || tstate == SPREFIX) && stat(errbuf, &sbuf) == 0 && (sbuf.st_mode & S_IFMT) == S_IFDIR)
			strcat(errbuf, "/");
 ...
 */
void
crdir(struct xfer *unused, struct transaction *t)
{
...
#if defined(MAP_ROOT_DIRECTORY)
			// each parsepath call adds treeroot, so remove one
		char *np = hidetreeroot(parent);
		free(parent);
		parent = np;
#endif
		if (access(parent, R_OK|W_OK) != 0) {
			if (errno == EACCES) {
				errcode = ATD;
 ...
 */
void
expunge(struct xfer *unused, struct transaction *t)
{
 ...
 */
void
chngprop(struct xfer *unused, struct transaction *t)
{
	struct xfile_handle *f = t->t_fh;
	struct xfer *x = XNULL;
	int errcode;
	...
	if (f != FNULL) {
		fhname = f->f_name;
		if (t->t_args != ANULL && t->t_args->a_strings[0] != NOSTR && t->t_args->a_strings[0][0] != '\0') {
			errstring = "Both file handle and file name in CHANGE-PROPERTIES";
			error(t, fhname, BUG);
		} else if ((x = f->f_xfer) == XNULL) {
			errstring = "No transfer on file handle for CHANGE-PROPERTIES";
			error(t, fhname, BUG);
		} else if (f->f_xfer->x_options & (O_PROPERTIES | O_XDIRECTORY)) {
			...
			for (pp = properties; pp->p_indicator; pp++)
				if (strcmp(pp->p_indicator, plp->p_name) == 0) {
					if (pp->p_put) {
						...
						errstring = errbuf;
						sprintf(errstring, "No a changeable property: %s", plp->p_name);
						error(t, fhname, CSP);
						return;
					}
				}
			if (pp->p_indicator == NOSTR) {
				sprintf(errbuf, "Unknown property name: %s", plp->p_name);
				errstring = errbuf;
				error(t, fhname, UKP);
				return;
			}
...
getspace(struct stat *s, char *cp)
{
	...
	while ((len = read(fd, (char *)&mtab, sizeof(mtab))) == sizeof(mtab)) {
		...
		if (stat(dev, &mstbuf) == 0 && mstbuf.st_rdev == s->st_dev) {
			break;
		}
	}
	close(fd);
	...
	free = sblock.fs_cstotal.cs_nbfree * sblock.fs_frag + sblock.fs_cstotal.cs_nffree;
#else
	...
	sprintf(cp, "%s (%s): %ld free, %ld/%ld used (%ld%%)", mtab.path, mtab.spec, free, used, total, (100L * used + total / 2) / total);
	while (*cp)
		cp++;
	return cp;
}
...
getmdate(struct stat *s, char *cp)
{
	...
	if (tm->tm_year > 99)
		tm->tm_year %= 100;
#endif
	sprintf(cp, "%02d/%02d/%02d %02d:%02d:%02d", tm->tm_mon + 1, tm->tm_mday, tm->tm_year, tm->tm_hour, tm->tm_min, tm->tm_sec);
	while (*cp)
		cp++;
	return cp;
}
...
getcdate(struct stat *s, char *cp)
{
	...
	tm = localtime(&s->st_mtime);
	sprintf(cp, "%02d/%02d/%02d %02d:%02d:%02d",
...
getname(struct stat *s, char *cp)
{
	...
	if ((pw = getpwuid(s->st_uid)) != NULL)
		sprintf(cp, "%s", pw->pw_name);
	else
		sprintf(cp, "#%d", s->st_uid);
	while (*cp)
		cp++;
	return cp;
}
...
{
	sprintf(cp, "0%o", s->st_mode & 07777);
	while (*cp)
		cp++;
	return cp;
}
...
int
putprot(struct stat *s, char *file, char *newprot, struct xfer *x)
{
	char *cp;
	int mode;
	...
	for (mode = 0, cp = newprot; *cp; cp++)
		if (*cp < '0' || *cp > '7') {
			...
			sprintf(errstring = errbuf, "Illegal protection mode, must be octal: %s", newprot);
			return IPV;
		} else {
			mode <<= 3;
			mode |= *cp & 7;
...
int
putname(struct stat *s, char *file, char *newname, struct xfer *x)
{
	struct passwd *pw;

	if ((pw = getpwnam(downcase(newname))) == NULL) {
		sprintf(errstring = errbuf, "Unknown user name: %s", newname);
		return IPV;
	} else if (s->st_uid != pw->pw_uid && chown(file, pw->pw_uid, s->st_gid) < 0) {
...
putmdate(struct stat *s, char *file, char *newtime, struct xfer *x)
{
	...
	if (mtime != s->st_mtime) {
		...
		if (utime(file, &timep) < 0) {
			errstring = "No permission to change modification time";
			return ATF;
		} else if (x) {
			x->x_mtime = mtime;
			x->x_flags |= X_MTIME;
...
parsetime(char *cp, time_t * t)
{
	...
	if (!(cp = tnum(cp, '/', &month)) || !(cp = tnum(cp, '/', &day)) || !(cp = tnum(cp, ' ', &year)) || !(cp = tnum(cp, ':', &hour)) || !(cp = tnum(cp, ':', &minute)) || !(cp = tnum(cp, '\0', &second)) || month < 1 || month > 12 || day < 1 || day > 31 || year < 70 ||	/* year > 99 || */
	    hour < 0 || hour > 23 || minute < 0 || minute > 59 || second < 0 || second > 59)
		return 1;
	if (year < 100)
		year += 1900;
	*t = 0;
/* @@@@ WARNING: this doesn't work every 400 years! */
...
parsepath(char *path, char **dir, char **real, int blankok)
{
	...
	{
		if ((treeroot == NULL) ||
		    ((treeroot_prefix != NULL) && strncasecmp(path,treeroot_prefix,strlen(treeroot_prefix)) != 0))
			wd = "";
		else {
			size_t newlength = strlen(treeroot) + strlen(path) + 2;

			if (treeroot_prefix == NULL) {
				/* Mapping root - make sure no unplanned escapes */
				int pl = strlen(path);
				if ((strstr(path,"/../") != NULL) || 
				    ((pl > 2) && 
					path[pl-3] == '/' && path[pl-2] == '.' && path[pl-1] == '.')) {
					errstring = "Relative directory path not allowed";
					return IPS;
				}
			}
			wd = malloc(newlength);
			...
			strcpy(wd, treeroot);
			if (treeroot_prefix != NULL)
				path += strlen(treeroot_prefix);
		}
	}
#else				/* !MAP_ROOT_DIRECTORY */
		wd = "";
#endif
	...
	if ((cp = malloc((unsigned)(strlen(wd) + strlen(path) + 2))) == NOSTR)
		fatal(NOMEM);
	strcpy(cp, wd);
	if ((wd[0] != '\0') && (*cp != '/')) /* Unless there already is one, add it */
		strcat(cp, "/");
	strcat(cp, path);
	// @@@@ NOTE: there is a bug in dcanon: /fsroot//tree/network/chaos/* becomes /fsroot//tree/etwork/chaos/*
	if ((errcode = dcanon(cp, blankok)) != 0) {
		free(cp);
		return errcode;
	}
	*real = cp;
	*real = case_insensitive_path(*real);
 ...
 */
int
fherror(struct xfile_handle *f, int code, int type, const char *message)
{
	struct file_error *e = &errors[code];
	struct chpacket pkt;

	pkt.cp_op = ASYNOP;
	sprintf(pkt.cp_data, "TIDNO %s ERROR %s %c %s", f->f_name, e->e_code, type, message ? message : e->e_string);
#ifdef SELECT
	struct packet *chpkt = chaos_allocate_packet(f->f_connection, pkt.cp_op, (int)strlen(pkt.cp_data));

	memcpy(chpkt->pk_cdata, pkt.cp_data, strlen(pkt.cp_data));
#ifdef USE_CBRIDGE
	// Multiple return values possible
	int q = chaos_connection_queue(f->f_connection, chpkt);
	if (q != 0)
	  return q;		// some kind of error, don't actually write the pkt
#else
	chaos_connection_queue(f->f_connection, chpkt);
#endif
#else
	int len = 1 + strlen(pkt.cp_data);
...
dowork(struct xfer *x)
{
	...
	case O_XDIRECTORY:
		...
		case X_DONE:
			 ...
			 */
#if CBDEBUG
		  fprintf(stderr,"%s: got DONE state, calling xclose [%p]\n", __func__, (void *)pthread_self());
#endif
			xclose(x);
			return X_SYNCMARK;
		case X_BROKEN:
		...
		case X_REOF:
		  ...
		  switch (xpweof(x)) {
		  case -2:
		    // case -2: we're interrupted, so set X_DONE
#if CBDEBUG
		    fprintf(stderr,"%s: got interrupt value from xpweof\n",__func__);
#endif
		    x->x_state = X_DONE;
		    break;
		  case -1:
		...
		while (x->x_room != 0) {
			if (x->x_left == 0) {
				...
				n = x->x_options & O_XDIRECTORY ? dirread(x) : x->x_options & O_PROPERTIES ? propread(x) : read(x->x_fd, x->x_bbuf, FSBSIZE);
				...
				case 0:
				  ...
				  case -2:
				    // case -2: we're interrupted, so set X_DONE
#if CBDEBUG
				    fprintf(stderr,"%s: got interrupt value from xpwrite (case 1) [%p]\n",__func__, (void *)pthread_self());
#endif
				    x->x_state = X_DONE;
				    break;
				  case -1:
		...
		case -2:
		  // case -2: we're interrupted, so set X_DONE
#if CBDEBUG
		  fprintf(stderr,"%s: got interrupt value from xpwrite (case 2) [%p]\n",__func__, (void *)pthread_self());
#endif
		  x->x_state = X_DONE;
		  break;
		case -1:
		...
		case X_PROCESS:
			{
				...
				case 0:
					...
					return X_CONTINUE;
				case -1:
					fherror(x->x_fh, NET, E_FATAL, "Data connection error");
					x->x_state = x->x_flags & X_CLOSE ? X_DONE : X_BROKEN;
					return X_CONTINUE;
				case -2:
					/*
			...
			if (x->x_room == 0) {
				if (xbwrite(x) >= 0) {
					...
					x->x_room = FSBSIZE;
				      ...
				      writerr:
					if (errno == ENOSPC && (x->x_flags & X_CLOSE) == 0) {
						fherror(x->x_fh, NMR, E_RECOVERABLE, "File system out of space");
						x->x_state = X_ERROR;
					} else {
						fherror(x->x_fh, MSC, E_FATAL, strerror(errno));
						x->x_state = X_WSYNC;
					}
				}
			}
...
#ifdef SELECT
	struct packet *chpkt = chaos_allocate_packet(x->x_fh->f_connection, op, 0);

#ifdef USE_CBRIDGE
	// Multiple return values possible
	int q = chaos_connection_queue(x->x_fh->f_connection, chpkt);
	if (q != 0)		// interrupt or some kind of error, don't write the pkt
	  return q;
#else
	if (chaos_connection_queue(x->x_fh->f_connection, chpkt))
		return -1;
#endif
#else
...
#ifdef SELECT
	struct packet *chpkt = chaos_allocate_packet(x->x_fh->f_connection, x->x_op, len - 1);

	memcpy(chpkt->pk_cdata, x->x_pkt.cp_data, len - 1);
#ifdef USE_CBRIDGE
	// Multiple return values possible - just return it
	return chaos_connection_queue(x->x_fh->f_connection, chpkt);
#else
	if (chaos_connection_queue(x->x_fh->f_connection, chpkt))
		return -1;
#endif
...
xpread(struct xfer *x)
{
...
#ifdef SELECT
	{
		...
		chpkt = chaos_connection_dequeue(x->x_fh->f_connection);
		if (chpkt == NOPKT)
			return -1;
		syslog(LOG_INFO, "FILE: read pkt (%#o) from net\n", chpkt->pk_op);
		switch (chpkt->pk_op) {
		case EOFOP:{
				syslog(LOG_INFO, "xpread: EOF\n");
				struct packet *status = chaos_allocate_packet(x->x_fh->f_connection, STSOP, 2 * sizeof(unsigned short));

				status->pk_pkn = chpkt->pk_pkn;
				*(unsigned short *)&status->pk_cdata[0] = x->x_fh->f_connection->cn_rlast;
				*(unsigned short *)&status->pk_cdata[2] = x->x_fh->f_connection->cn_rwsize;
		...
		case DATOP:
		case DWDOP:
			if (PH_LEN(chpkt->pk_phead) == 0) {
				syslog(LOG_INFO, "FILE: zero size data packet\n");
				goto loop;	/* Zero size data packet!? */
			}
			syslog(LOG_INFO, "xpread: DATA length=%d\n", PH_LEN(chpkt->pk_phead));
			n = PH_LEN(chpkt->pk_phead) & 0x3ff;
			x->x_pkt.cp_op = chpkt->pk_op;
			memcpy(x->x_pkt.cp_data, chpkt->pk_cdata, PH_LEN(chpkt->pk_phead));
			free(chpkt);
			chpkt = NOPKT;
			return n;
		default:
			syslog(LOG_INFO, "FILE: bad opcode in data connection: %d\n", x->x_op & 0377);
			free(chpkt);
			chpkt = NOPKT;
...
finish(void)
{
	...
	if (getpid() == mypid && (ufd = open(FILEUTMP, 1)) >= 0) {
		mylogin.cl_user[0] = '\0';
		lseek(ufd, (long)(mylogin.cl_cnum * sizeof(mylogin)), 0);
		write(ufd, (char *)&mylogin, sizeof(mylogin));
		close(ufd);
	}
	/*
...
#endif
{
	...
	for (;;) {
		...
		syslog(LOG_INFO, "Switch pos: %d, status: %ld\n", tell(x->x_fd), x->x_state);
		switch (dowork(x)) {
		case X_SYNCMARK:
#if CBDEBUG
		  fprintf(stderr,"%s: got SYNCMARK from dowork [%p]\n", __func__, (void *)pthread_self());
#endif
		...
		case X_HANG:	/* Need more instructions */
			syslog(LOG_INFO, "Hang pos: %d\n", tell(x->x_fd));
			continue;
		}
		syslog(LOG_INFO, "FILE: subproc exiting\n");
#if CBDEBUG
		  fprintf(stderr,"%s: calling xflush [%p]\n", __func__, (void *)pthread_self());
#endif
		xflush(x);
		return (void *) 0;
	}
	...
	case -1:
		...
		return NER;
	default:
		if (!(x->x_options & (O_PROPERTIES | O_XDIRECTORY)))
			close(x->x_fd);
		x->x_fd = -1;
		close(pfd[0]);
		return 0;
	...
	case 0:
		for (i = 3; i < 20; i++)
			if (i != pfd[0] && i != x->x_fh->f_fd && i != ctlpipe[1] && i != x->x_fd)
				close(i);
		x->x_pfd = pfd[0];
		myxfer = x;
		setjmp(closejmp);
		for (;;) {
			...
			case X_SYNCMARK:
#if CBDEBUG
		  fprintf(stderr,"%s: got SYNCMARK from dowork\n", __func__);
#endif
				syncmark(x->x_fh);	/* Ignore errors */
				break;
			case X_FLUSH:	/* Totally done */
...
struct transaction *
makecmd(struct command *c, struct xfile_handle *f)
{
...
sendcmd(struct transaction *t, struct xfer *x)
{
	...
	if ((a = t->t_args) != NULL) {
		...
		pm.pm_args = 0;
	if (write(x->x_pfd, (char *)&pm, sizeof(pm)) != sizeof(pm) || a && a->a_strings[0] && write(x->x_pfd, a->a_strings[0], (unsigned)pm.pm_strlen) != pm.pm_strlen) {
...
rcvcmd(struct xfer *x)
{
	...
	if ((n = read(x->x_pfd, (char *)&pm, sizeof(pm))) != sizeof(pm))
	...
	if (pm.pm_args) {
		...
		if (pm.pm_strlen) {
			if ((t->t_args->a_strings[0] = malloc(pm.pm_strlen + 1)) == NOSTR)
				fatal(NOMEM);
			if ((n = read(x->x_pfd, t->t_args->a_strings[0], pm.pm_strlen)) != pm.pm_strlen)
...
{
#ifdef USE_CBRIDGE
	// make sure the interruption lock is unlocked when starting a new xfer
	if ((ax->x_fh != NULL) && (ax->x_fh->f_connection != NULL)) {
	  chaos_clear_connection_interrupt(ax->x_fh->f_connection);
	}
#if CBDEBUG
	else {
	  fprintf(stderr,"%s: can't clear interruption: x_fh %p, f_conn %p\n",
		  __func__, ax->x_fh, ax->x_fh != NULL ? ax->x_fh->f_connection : NULL);
	}
#endif
#endif
	// make sure the thread is detached from the start
	int e;
	pthread_attr_t attr;
	...
	pthread_create(&startxfer_thread, &attr, _startxfer, (void *)ax);
	return 0;
}

void
processdata(struct connection *conn)
{
#ifdef USE_CBRIDGE
  // clear startxfer_thread, just to play safe
  memset(&startxfer_thread, 0, sizeof(startxfer_thread));
  _processdata(conn);
#else
	// make sure the thread is detached from the start
	int e;
	...
	pthread_create(&processdata_thread, &attr, _processdata, (void *)conn);
#endif
}
#endif

// The absolute dumpster fire case correction
static int depth(char *path) {
	int level = -1;
	for (int i = 0; i < strlen(path); i++) {
		if (path[i] == '/') level++;
	}
	if (path[strlen(path) - 1] == '/') level--;
	return level;
}
...
static char *find_case_insensitive(char *directory, char *name) {
	...
	if (dp == NULL) {
		if (errno == EACCES) {
			...
			fprintf(stderr, "%s is not a directory.", directory);
		} else {
			fprintf(stderr, "Unknown error opening directory in find_case_insensitive.\n");
		}
		closedir(dp);
		return NULL;
	}
	char *downcaseName = malloc(strlen(name) + 1);
	bzero(downcaseName, strlen(name) + 1);
	strcpy(downcaseName, name);
	downcase(downcaseName);
	char *entryName = malloc(NAME_MAX);
	while ((dirEntry = readdir(dp)) != NULL) {
		strcpy(entryName, dirEntry->d_name);
		downcase(entryName);
		if (strcmp(entryName, downcaseName) == 0) {
			strcpy(entryName, dirEntry->d_name);
			free(downcaseName);
			closedir(dp);
			return entryName;
...
static char *getatdepth(char *path, int level) {
	...
	int p = 0;
	int len = strlen(path);
	for (int i = 0; i < len && l != level; i++) {
		p = i;
		if (path[i] == '/') l++;
	}
	int sublen = 0;
	for (int i = p+1; i < len && path[i] != '/'; i++) {
		sublen = i - p;
	}
...
char *case_insensitive_path(char *path) {
	char *sofar = malloc(strlen(path) + 1);
	bzero(sofar, strlen(path) + 1);
	sofar[0] = '/';
	int levels = depth(path);
	for (int i = 0; i <= levels; i++) {
		char *subpath = getatdepth(path, i);
		char *actual = find_case_insensitive(sofar, subpath);
		if (actual == NULL) {
			bzero(sofar, strlen(path) + 1);
			strcpy(sofar, path);
			return sofar;
