...
static int tmount(void);
static int tprobe(void);
static int tread(void);
static int twrite(void);
static int trewind(void);
static int trewsync(void);
static int toffline(void);
static int tfile(void);
static int tblock(void);
static int tweof(void);
...
#ifdef SELECT
void *
_processrtape(void *conn)
{
}

pthread_t processrtape_thread;

void
processrtape(struct connection *conn)
{
	pthread_create(&processrtape_thread, NULL, _processrtape, (void *)conn);
}
#else
int
...
tmount(void)
{
	 ...
	 */
	tapemode = cp == NULL ? -1 : !strcmp(type, "READ") ? 0 : !strcmp(type, "WRITE") ? 2 : !strcmp(type, "BOTH") ? 2 : -1;
	if (tapemode == -1) {
	...
	if (drive[0] == '\0')
		...
		tstatus(0, 0, "Drive can only be single digits 0-3: %s", tapemode);
		return;
	} else
		drivenum = drive[0] - '0';
	if (blocksize[0] == '\0')
	...
	if ((tapefd = open(tapepath, tapemode)) < 0) {
		...
		case ENXIO:
			...
			break;
		case EIO:
			tstatus(0, 0, "Drive open error (Offline, No Write ring etc.) on drive: %d", drivenum);
			break;
		case EPERM:
			tstatus(0, 0, "No permission to access drive: %s", drive);
			break;
		default:
			tstatus(0, 0, "System error %d while opening drive: %d", errno, drivenum);
		}
		return;
	}
	{
...
int
parseoptions(char *string, int *bits)
{
	char *cp, *op;
	struct option *o;
 ...
 */
void
tstatus(int id, int hard, char *string, ...)
{
	va_list ap;
	...
	if (string) {
		ts.t_message = 1;
		vsprintf(ts.t_string, string, ap);
	}
	if (!ts.t_message)
		ts.t_string[0] = '\0';
	if (recwrite(rs, STATUS, (char *)&ts, (int)((struct tape_status *)0)->t_string + strlen(ts.t_string)) == EOF)
		fatal(recbad);
	va_end(ap);
	recforce(rs);
}
...
int
numrec(struct record_stream *rs)
{
	int n = 0;
	int c;
	int sign = 0;
